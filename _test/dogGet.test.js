...
describe('Dogs get Testing Cases', () => {
  it('get all dog information', async () => {
    const res = await request(app.callback())
      .get('/api/v1/dog')      
      .send({})
    expect(res.statusCode).toEqual(200)
    expect(res.type).toEqual("application/json")
    expect(res.body).toEqual(expectedAllDogs)
  })
  it('get one dog information', async () => {
    const res = await request(app.callback())
      .get('/api/v1/dog/6289f075f2f0ccbc448b1a71')
      .send({})
    expect(res.statusCode).toEqual(200)
    expect(res.type).toEqual("application/json")
    expect(res.body).toEqual(expectedDog)
  })
})